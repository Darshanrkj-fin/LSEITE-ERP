...
const LABELS = {
  sales: { title: 'New Sales Invoice', partyType: 'customer', partyLabel: 'Customer', accountType: 'income', accountLabel: 'Revenue account' },
  purchase: { title: 'New Purchase Invoice', partyType: 'vendor', partyLabel: 'Vendor', accountType: 'expense', accountLabel: 'Expense account' },
}

const today = () => new Date().toISOString().slice(0, 10)

const emptyLine = { item_id: '', quantity: '1', rate: '', expiry_date: '' }

export function InvoiceForm({ type, basePath }) {
  ...
  useEffect(() => {
    async function loadOptions() {
      const [{ data: partyRows }, { data: accountRows }, { data: itemRows }, { data: company }, { data: orderRows }] = await Promise.all([
        supabase.from('parties').select('id, name, state_code').eq('type', labels.partyType),
        supabase.from('chart_of_accounts').select('id, name').eq('type', labels.accountType),
        supabase.from('items').select('id, name, hsn_sac_code, item_type'),
    ...
    loadOptions()
  }, [labels.partyType, labels.accountType, profile.company_id, type])
  ...
  const handleSubmit = async (e) => {
    ...
    if (!partyId || !accountId) {
      setError(`Select a ${labels.partyLabel.toLowerCase()} and a ${labels.accountLabel.toLowerCase()}.`)
      return
    }
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200 text-left text-slate-500">
              ...
              {type === 'purchase' && <th className="py-2 pr-4">Expiry (raw material)</th>}
            ...
            {lines.map((line, i) => {
              ...
              return (
                <tr key={i} className="border-b border-slate-100">
                  ...
                  {type === 'purchase' && (
                    <td className="py-2 pr-4">
                      {items.find((it) => it.id === line.item_id)?.item_type === 'raw_material' && (
                        <input
                          type="date"
