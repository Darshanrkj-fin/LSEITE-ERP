...
// Shared by api/invoice-pdf.js (download), api/email-invoice-pdf.js (email
// attachment), and api/quote-pdf.js so none of them drift out of sync.
// company must include logo_url/udyam_number if you want them printed —
// both optional. heading/numberLabel let a quote reuse this layout
// (heading: 'QUOTATION', numberLabel: 'Quote #') without a second
// near-identical PDF builder — quotes and invoices share the same shape
// (party, dated line items with the same GST columns, subtotal/cgst/sgst/
// igst/grand_total).
export async function buildInvoicePdf({ invoice, lineItems, company, heading, numberLabel = 'Invoice #' }) {
  ...
  text(heading ?? (invoice.type === 'sales' ? 'TAX INVOICE (Sales)' : 'PURCHASE INVOICE'), left, 13, true)
  y -= 18
  text(`${numberLabel}: ${invoice.invoice_number}`, left, 10)
  y -= 14
